...
elseif ($_REQUEST['act'] == 'edit_goods_weight')
{
    ...
    if($goods_weight < 0 || $goods_weight == 0 && $_POST['val'] != "$goods_weight")
    {
        make_json_error($_LANG['goods_weight_error']);
    }

    if(check_goods_product_exist($goods_id) == 1)
    {
        make_json_error($_LANG['sys']['wrong'] . $_LANG['cannot_goods_weight']);
    }

    if ($exc->edit("goods_weight = '$goods_weight', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($goods_weight);
    }
}
...
//-- 修改包邮状态
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_shipping')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $shipping        = intval($_POST['val']);

    if ($exc->edit("is_shipping = '$shipping', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($shipping);
    }
}
